...
export default class Month extends React.Component {
    ...
    render() {
        let {
            days, changeSelection, style, monthsLocale, daysTextColor,
            bodyBackColor, bodyTextColor, headerSepColor, width, holidaysTextColor
        } = this.props;
                    ...
                    {this.weekDaysLocale.map((dayName, i) => {
                        return (
                            <View key={i}
                                ...
                                <Text style={{ color: i < 5 ? daysTextColor : holidaysTextColor }}>{dayName}</Text>
                            </View>
                        );
                    })}
                    {days.map((day, i) => {
